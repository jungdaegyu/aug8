...
$(function(){
	...
	$("#idCheck").click(function(){ //중복검사를 클릭하면
		...
		if(id == "" || id.length < 5){ //5글자보다 적게 입력하면
			$("#id").focus();
			$("#resultMSG").text("아이디는 다섯글자 이상이어야합니다.");//출력
			...
			$.ajax({ //ajax 호출 및 시작
				...
				type : "post",
				data : {"id" : id},
				dataType : "json", //{result : 0} 이런식으로 보내줄거임
				...
				},//여기에 있는 데이터는 서버에서 오는거지 위의 data가 아님
				error: function(request, status, error){
					$("#resultMSG").text("오류가 발생했습니다. 가입할 수 없습니다. " + error);
				}
			});	
		}
		return false; //멈추기
